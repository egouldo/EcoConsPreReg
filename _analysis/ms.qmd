...
```{r}
#| label: tbl-preregistration-template-final
#| tbl-cap: "Final version of preregistation template developed after user-testing."
...
prereg_tbl_data <- 
  readr::read_csv(
    ...
    group_by(phase, step, substep) %>% 
    mutate(preregistration_item_id = row_number()) %>% 
    unnest(data) %>% ungroup %>% 
    nest(.by = c(phase, 
                 step, 
                 substep, 
    ...
    group_by(phase, step, substep, preregistration_item) %>% 
  mutate(preregistration_subitem_id = row_number()) %>% 
    unnest(data) %>% 
  ungroup %>%  
    mutate(preregistration_item = 
             case_when(preregistration_subitem_id != 1 ~ NA, 
                       .default = preregistration_item),) %>% 
    mutate(substep = 
             case_when(substep_id != row_number() ~ NA, 
                               .default = substep),
           substep_description = 
             case_when(substep_id != row_number() ~ NA, 
                       .default = substep_description), 
           .by = substep) %>% 
    mutate(step = 
             case_when(step_id != row_number() ~ NA, 
                            .default = step),
           step_description = 
             case_when(substep_id != row_number() ~ NA, 
                                        .default = step_description),
           .by = step)  %>% 
    mutate(across(everything(), ~ stringr::str_remove_all(.x, "\\*")),
           phase = gluedown::md_bold(phase),
           phase_description = gluedown::md_italic(phase_description),
           across(ends_with("_id"), as.numeric))

prereg_tbl_data %>% 
    select(starts_with("phase"), starts_with("step"), starts_with("substep"), ends_with("_item"), ends_with("subitem"), everything()) %>% 
    mutate(step = case_when(!is.na(step) ~ gluedown::md_bold(step), .default = step),
           step_description = case_when(!is.na(step_description) ~ gluedown::md_italic(step_description), .default = step_description)) %>% 
    gt::gt(groupname_col = c("phase", "phase_description"),
           row_group.sep = ": ") %>% 
   ...
   gt::text_transform(fn = gt::md, 
    ...
    gt::cols_merge(columns = c("step", "step_description"), pattern = "{1} {2}") %>% 
    gt::cols_merge(columns = c("substep", "substep_description"), pattern = "{1} {2}") 
```
