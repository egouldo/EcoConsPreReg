...
suppressPackageStartupMessages(library(magick, quietly = TRUE))
...
#| column: body
#| fig-width: 10
#| fig-cap: "Preregistration as a component of the registered report publication pathway, as illustrated by [@Pu2019]."
...
When preliminary analyses, model checking or other data-dependent decisions need to be made, the modeller can specify a 'flexible strategy' (@fig-registering-flexibility) in their preregistration by:
...
::: {#fig-registering-flexibility .content-visible when-format="html"}
```{mermaid}
%%| column: body-outset
---
config:
...
---
flowchart LR
  A(Describe Data-Dependent Decision) --> B{Describe Analysis}
  B --> C[Analysis Outcome 1]
  B --> D[Analysis Outcome 2]
  C --> E(Option 1)
  D --> F(Option 2)
```

A general heuristic for registering flexible analyses.
:::

::: {#fig-registering-flexibility .content-visible when-format="pdf"}
```{mermaid}
%%| fig-width: 6
%%| column: body-outset
---
...
magick::image_read_pdf(path = "figures/interim_preregistrations.pdf") %>%
  magick::image_resize(geometry = "35%")
```
...
magick::image_read(path = "figures/LivingPreregistration.png",
                   density = 1000) %>%
  magick::image_scale("80%")
```
:::
